...
export default function Data({ data = [] }) {
  ...
  const handleFilterData = (e) => {
    ...
    const filterFunction = (item) => {
      return (
        value === "" ||
        item.track.name.toLowerCase().includes(value.toLowerCase())
      );
    };
  ...
  const handleSortData = (sort) => {
    const sortData = [...filterData].sort((a, b) => {
      if (sort === "title") {
        ...
        return a?.track?.artists[0]?.name?.localeCompare(
          b?.track?.artists[0]?.name
        );
      } else if (sort === "album") {
        return a?.track?.album?.name?.localeCompare(b?.track?.album?.name);
      } else if (sort === "recent") {
        return b["added_at" || "played_at"].localeCompare(
          a["added_at" || "played_at"]
        );
        ...
        <Modal onClose={() => setIsOpen(false)} isOpen={isOpen}>
          <div className="select-sort">
            ...
            <p onClick={() => handleSortData("duration")}>
              <span>duration</span>{" "}
              <span>{selectSort === "duration" && "✔"}</span>
            </p>
            <p onClick={() => handleSortData("recent")}>
              <span>recent Added</span>{" "}
              <span>{selectSort === "recent" && "✔"}</span>
            </p>
          </div>
        </Modal>
      </div>
